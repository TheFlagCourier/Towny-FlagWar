...
public class FlagWar {
	...
	public static void registerEvents() {
		 ...
		 *  - FlagWarBlockListener
		 *  - FlagWarCustomListener  << Merged TownyCommonListener into FWCL
		 *  - FlagWarEntityListener
		 */
	...
	public static void registerAttack(CellUnderAttack cell) throws TownyException {
		...
		if (futureActiveFlagCount > io.github.townyadvanced.flagwar.FlagWarConfig.getMaxActiveFlagsPerPerson())
			throw new TownyException(Translation.of("msg_err_enemy_war_reached_max_active_flags", io.github.townyadvanced.flagwar.FlagWarConfig
	...
	public static void checkBlock(Player player, Block block, Cancellable event) {
		...
		if (io.github.townyadvanced.flagwar.FlagWarConfig.isAffectedMaterial(block.getType())) {
	...
	public static boolean callAttackCellEvent(Towny plugin, Player player, Block block, WorldCoord worldCoord) throws TownyException {
		...
		if (io.github.townyadvanced.flagwar.FlagWarConfig.isAttackingBordersOnly() && !AreaSelectionUtil.isOnEdgeOfOwnership(landOwnerTown, worldCoord))
		...
		double costToPlaceWarFlag = io.github.townyadvanced.flagwar.FlagWarConfig.getCostToPlaceWarFlag();
		if (TownySettings.isUsingEconomy()) {
			try {
				...
				double defendedAttackCost = io.github.townyadvanced.flagwar.FlagWarConfig.getDefendedAttackReward() * (activeFlagCount + 1);
				double attackWinCost = 0;

				double amount;
				amount = io.github.townyadvanced.flagwar.FlagWarConfig.getWonHomeblockReward();
				double homeBlockFine = amount < 0 ? -amount : 0;
				amount = io.github.townyadvanced.flagwar.FlagWarConfig.getWonTownblockReward();
	...
	public static void checkIfTownHasMinOnlineForWar(Town town) throws TownyException {
		...
		int requiredOnline = io.github.townyadvanced.flagwar.FlagWarConfig.getMinPlayersOnlineInTownForWar();
